...
export async function* readLinesSafe(filePath: string): AsyncGenerator<string> {
...
export function getFailedLogPath() {
  ...
  try {
    ...
    try {
      for await (const line of readLinesSafe(processingFile)) {
        ...
        console.error('Error processing retry loop', err);
    }
