...
- `GET /readiness`: Readiness der **kritischen** Chronik-Senke (`agent.ledger`). Internes Diagnostik-Signal für Operator/Leitstand, das die kritische Teilmenge der Queue isoliert von Best-Effort-/Legacy-Fehlern zeigt.
  ...
  - Felder: `status_basis`, `active_probe`, `configured`, `queued`, `retryable_now`, `next_due_at`, `due_now`, `last_error`, `last_delivered_at`.
  ...
  - `last_error` reflektiert den **aktuellen** Fehlerzustand und wird aus der Queue rekonstruiert: bei `degraded` der Fehler eines noch offenen kritischen Eintrags (auch nach Neustart), bei leerer kritischer Queue bereinigt (`null`).
  ...
  - **Abgrenzung (Doktrin):** `/readiness` ist Plexers eigenes Diagnostik-Signal, **nicht** der `plexer.delivery.report.v1`-Contract, **kein Producer-Gate** und **kein Kubernetes-/Load-Balancer-`readinessProbe`**. Ein `degraded`/`unconfigured` Zustand heißt nicht, dass Producer aufhören sollen zu senden oder Plexer aus der Rotation genommen werden soll — Plexer puffert die operativen Events weiter für den Retry (Relay degradiert, ohne die Task-Wahrheit zu ändern).
