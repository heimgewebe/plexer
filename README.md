...
- `GET /readiness`: Readiness der **kritischen** Chronik-Senke (`agent.ledger`). Internes Diagnostik-Signal für Operator/Leitstand, das die kritische Teilmenge der Queue isoliert von Best-Effort-/Legacy-Fehlern zeigt.
  ...
  - Felder: `status_basis`, `active_probe`, `configured`, `queued`, `retryable_now`, `next_due_at`, `last_error`, `last_delivered_at`.
  ...
  - `last_error` reflektiert den **aktuellen** Fehlerzustand: nach erfolgreicher Zustellung bzw. leerer kritischer Queue wird es bereinigt (`null`).
